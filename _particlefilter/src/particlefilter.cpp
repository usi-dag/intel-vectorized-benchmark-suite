...
*/
inline _MMR_f64 randu_vector(double * seed, int index , double* result, double* num)
{
    /*
    ...
    */
    ...
    for(int x = index; x < index+SPECIES_64; x++){


        num[x-index] = A* (int) seed[x] + C;
        seed[x] = fmod(num[x-index], M);
        result[x-index] = fabs(seed[x]/(M));
    }
    _MMR_f64    xResult;
//     TODO loading result return a vector with n-1 elem all 0
    xResult = _MM_LOAD_f64(&result[0]);
//    FENCE();
//    print_vector_64(xResult);
    return xResult;
}
...
#ifdef USE_VECTOR_INTRINSIC
inline _MMR_f64 randn_vector(double * seed, int index ,double* randu_vector_result,double* randu_vector_num){
    /*Box-Muller algorithm*/
...
void videoSequence(int *I, int IszX, int IszY, int Nfr, double *seed) {
    std::cout << "ENTER VIDEOSEQUENCE" << std::endl;
...
#ifdef USE_VECTOR_INTRINSIC
void particleFilter_vector(int * I, int IszX, int IszY, int Nfr, double * seed,double* randu_vector_result,double* randu_vector_num, int Nparticles){
...
//    }
    ...
    int limit = loop_bound(SPECIES_64, Nparticles);
//    unsigned long int gvl = __builtin_epi_vsetvl(Nparticles, __epi_e64, __epi_m1);
    ...
    x = 0;
//    for(x < Nparticles; x=x+SPECIES_64){
...
//    printf("TIME TO GET WEIGHTSTOOK: %f\n", elapsed_time(get_neighbors, get_weights));
    ...
    double * locations = (double *)malloc(sizeof(double *)*Nparticles);
...
//    gvl     = __builtin_epi_vsetvl(Nparticles, __epi_e64, __epi_m1);
    ...
    for(x = 0; x < limit; x=x+SPECIES_64){
...
//        gvl     = __builtin_epi_vsetvl(Nparticles, __epi_e64, __epi_m1);
        ...
        int * mask = (int *) malloc(SPECIES_64*sizeof(int *));

        for (int mi = 0; mi < SPECIES_64; mi++) {
            mask[mi] = 0;
        }
...
//                    printf("complete: %ld\n", vector_complete);
            ...
            _MM_STORE_f64(&locations[i],xArray);
...
//double *seed1;
//int *I1;
//double* randu_vector_result1;
//double* randu_vector_num1;
//
//static void DoSetup(const benchmark::State &state) {
//    std::cout << "ENTER SETUP" << std::endl;
//    seed1 = (double *) malloc(sizeof(double) * Nparticles1);
//    int i;
//    for (i = 0; i < Nparticles1; i++) {
//        seed1[i] = i; //time(0)*i;
//    }
//    //malloc matrix
//    I1 = (int *) malloc(sizeof(int) * IszX1 * IszY1 * Nfr1); // 128 * 128 * 10 = 163840 * sizeof(int)
//    //call video sequence
//    videoSequence(I1, IszX1, IszY1, Nfr1, seed1);
//
//#ifdef USE_VECTOR_INTRINSIC
//        double* randu_vector_result1 = (double*)malloc(SPECIES_64*sizeof(double));
//        double* randu_vector_num1    = (double*)malloc(SPECIES_64*sizeof(double));
//#endif
//}
//
...
//
//#ifdef USE_VECTOR_INTRINSIC
//    free(randu_vector_result1);
//    free(randu_vector_num1);
//#endif
//    free(seed1);
//    free(I1);
//}
...
BENCHMARK(BM_particlefilter)->Setup(DoSetup)->Unit(benchmark::kSecond)->MinWarmUpTime(0)->Repetitions(3)->ReportAggregatesOnly(false)->

DisplayAggregatesOnly(false)->Teardown(DoTeardown);

#else
BENCHMARK(BM_particlefilter)->Setup(DoSetup)->Unit(benchmark::kSecond)->MinWarmUpTime(0)->Repetitions(10)->ReportAggregatesOnly(false)->
