...
_MMR_f32 CNDF_SIMD  (_MMR_f32 xInput) {
  ...
  xMask       = _MM_VFLT_f32(xInput, xVLocal_2);  //_MM_VFLT_f32(src2,src1)
  xInput      = _MM_VFSGNJX_f32(xInput); //_MM_VFSGNJX_f32(src2,src1)
...
void BlkSchlsEqEuroNoDiv_vector (fptype * OptionPrice, fptype * sptprice,
                          ...
                          fptype * time, float * otype/*,long int *  otype_d*/, float timet, int i)
{
    ...
    xStrikePrice = _MM_DIV_f32(xStockPrice,xStrikePrice);
     xLogTerm = _MM_LOG_f32(xStrikePrice);
    ...
    xRatexTime = _MM_VFSGNJN_f32(xRatexTime);
...
fptype BlkSchlsEqEuroNoDiv( fptype sptprice,
                            fptype strike, fptype rate, fptype volatility,
                            fptype time, float otype, float timet )
{
  fptype OptionPrice;
...
int bs_thread(void *tid_ptr) {
    ...
    for (j=0; j<NUM_RUNS; j++) {
        int limit = loop_bound(SPECIES_32, numOptions);
#ifdef ENABLE_OPENMP
#pragma omp parallel for private(i, price, priceDelta)
        for (i=0; i<limit; i += SPECIES_32) {
#else  //ENABLE_OPENMP
        for (i=start; i<end; i += SPECIES_32) {
#endif //ENABLE_OPENMP
...
//    printf("PARSEC Benchmark Suite\n");
    ...
    nThreads = 1;
    std::string inputFileString = "input/in_512K.input";
    const char *inputFile = inputFileString.c_str();
...
#define LINESIZE 64
    ...
    otype = (float *) (((unsigned long long)buffer2 + PAD) & ~(LINESIZE - 1));

    for (i=0; i<numOptions; i++) {
        otype[i]      = (data[i].OptionType == 'P') ? 1.0 : 0.0; // TODO should be int  (but float as 128 and 256 load int instr not exists)
        sptprice[i]   = data[i].s;
...
BENCHMARK(BM_blackscholes)->Setup(DoSetup)->Unit(benchmark::kMillisecond)->MinWarmUpTime(20)->Iterations(10)->
