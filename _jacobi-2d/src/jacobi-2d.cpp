...
int TSTEPS = 14; // atoi(argv[2]);
string outfilename = "jacobi-2d output"; // argv[3];
...
//    unsigned long int gvl = __builtin_epi_vsetvl(size_y, __epi_e64, __epi_m1);
    int limit = loop_bound(SPECIES_64, size_y);

    xConstant = _MM_SET_f64(0.2);

    for (i = 1; i <= size_y; i++) {
        for (j = 1; j < limit; j += SPECIES_64) {
...
static void BM_jacobi2d(benchmark::State &state) {
    for (auto _: state)
#ifdef USE_VECTOR_INTRINSIC
...
BENCHMARK(BM_jacobi2d)
->
Setup(DoSetup)
->
Unit(benchmark::kMillisecond)
->MinWarmUpTime(20)->Iterations(10)->
Teardown(DoTeardown);
