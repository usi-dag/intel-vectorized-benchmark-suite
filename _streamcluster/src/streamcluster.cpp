...
//  unsigned long int gvl = __builtin_epi_vsetvl(dim, __epi_e32, __epi_m1);
    int limit = loop_bound(SPECIES_32, dim);
 _MMR_f32 result1, _diff, _coord1, _coord2;

  result1 = _MM_SET_f32(0.0);
//  result2 = _MM_SET_f32(0.0);
  for (i=0;i<limit;i=i+SPECIES_32) {
...
//    gvl = __builtin_epi_vsetvl(dim-i, __epi_e32, __epi_m1);
    ...
    result1   = _MM_MADD_f32(_diff,_diff, result1);
  }
...
//  double lastcost;
  ...
  //my block
  long bsize = numberOfPoints/nproc;
  long k1 = bsize * pid;
  long k2 = k1 + bsize;
  if( pid == nproc-1 ) k2 = numberOfPoints;
