...
double seed = 1979; //arbitrary (but constant) default value (birth year of Christian Bienia)
double swaption_seed;
double *swaption_seed_vector;
...
//        unsigned long int gvl = __builtin_epi_vsetvl(NUM_TRIALS, __epi_e64, __epi_m1);
        int limit = loop_bound(SPECIES_64, NUM_TRIALS);
        swaption_seed_vector = (FTYPE*)malloc(limit*sizeof(FTYPE));
        for(int j=0; j < limit; j++) {
            swaption_seed_vector[j] = swaption_seed + i;// + j + (i * limit);
        }
        BLOCK_SIZE_AUX = limit;
#else
        swaption_seed_vector = (FTYPE *) malloc(1 * sizeof(FTYPE));
...
//           nSwaptions);
    ...
    swaption_seed = (FTYPE) (2147483647L * RanUnif(&seed));
...
BENCHMARK(BM_swaptions)->Setup(DoSetup)->Unit(benchmark::kSecond)->MinWarmUpTime(20)->Repetitions(10)->Teardown(DoTeardown)->
