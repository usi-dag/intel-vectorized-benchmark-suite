...
//precompute the exponientials
//    for (j = 0; j <= (iN - 1) * BLOCKSIZE - 1; ++j) { printf("pdRate[%d] = %f\n", j, pdRatePath[j]);}
...
//	unsigned long int gvl = __builtin_epi_vsetvl(BLOCKSIZE, __epi_e64, __epi_m1);
    int limit = loop_bound(SPECIES_64, BLOCKSIZE);
  _MMR_f64 	xDdelt;
  ...
  //precompute the exponientials
//  printf("limit: %d\n", limit);
//  for (j = 0; j <= (iN - 1) * BLOCKSIZE - 1; ++j) { printf("pdRateV[%d] = %f\n", j, pdRatePath[j]);}
  for (j=0; j<=(iN-1)*BLOCKSIZE-1; j+=SPECIES_64) {
//      for (b=0; b < limit; b += SPECIES_64) {
          xpdRatePath = _MM_LOAD_f64(&pdRatePath[j]);

          xpdRatePath = _MM_MUL_f64(xpdRatePath,xDdelt);

          xpdRatePath = _MM_EXP_f64(_MM_VFSGNJN_f64(xpdRatePath));
          _MM_STORE_f64(&pdexpRes[j],xpdRatePath);
//      }
